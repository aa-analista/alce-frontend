...
const ACCEPT_ADJUNTOS = 'image/*,application/pdf,.doc,.docx,.xls,.xlsx,.txt,.csv'
...
export default function SoporteModule() {
  ...
  // Sube archivos a un ticket (opcionalmente colgados de un comentario)
  const subirAdjuntos = async (ticketId, fileList, comentarioId = null) => {
    if (!fileList || fileList.length === 0) return
    const fd = new FormData()
    fileList.forEach((f) => fd.append('archivos', f))
    if (comentarioId) fd.append('comentario_id', String(comentarioId))
    await fetch(`/api/soporte/${ticketId}/adjuntos`, { method: 'POST', headers: authHeaders, body: fd })
  }
  ...
  const crearTicket = async (e) => {
    ...
    try {
      ...
      // Sube los adjuntos al ticket recién creado
      if (files.length > 0) {
        try { await subirAdjuntos(data.id, files) } catch { /* no bloquea la creación */ }
      }
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      {vista === 'nuevo' && (
        <form onSubmit={crearTicket} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 space-y-4 max-w-2xl">
          ...
          {/* Adjuntos */}
          <div>
            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Adjuntos (opcional)</label>
            <label className="flex items-center gap-2 px-4 py-3 border border-dashed border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-500 dark:text-slate-400 cursor-pointer hover:border-[var(--brand-primary)]/40 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
              <Paperclip className="w-4 h-4" /> Adjuntar archivos, fotos o PDF
              <input type="file" multiple accept={ACCEPT_ADJUNTOS} className="hidden"
                onChange={(e) => { setFiles([...files, ...Array.from(e.target.files)]); e.target.value = '' }} />
            </label>
            {files.length > 0 && (
              <div className="space-y-1.5 mt-2">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between gap-2 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
                    ...
                    <button type="button" onClick={() => setFiles(files.filter((_, j) => j !== i))} className="text-slate-400 hover:text-red-500 shrink-0"><X className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="flex items-center justify-end gap-3">
            <button type="button" onClick={() => { setVista('lista'); setError(''); setFiles([]) }}
...
function TicketDetail({ data, isStaff, token, currentUser, onClose, onChanged }) {
  ...
  const enviarComentario = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // Sube adjuntos de la respuesta (colgados del comentario)
        if (comentFiles.length > 0) {
          const fd = new FormData()
          comentFiles.forEach((f) => fd.append('archivos', f))
          if (cdata.comentario_id) fd.append('comentario_id', String(cdata.comentario_id))
          try { await fetch(`/api/soporte/${data.id}/adjuntos`, { method: 'POST', headers: authHeaders, body: fd }) } catch { /* no bloquea */ }
        }
        setComentario(''); setInterno(false); setComentFiles([]); onChanged()
      }
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {data.adjuntos && data.adjuntos.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-wide text-slate-400 mb-2 flex items-center gap-1">
                <Paperclip className="w-3 h-3" /> Adjuntos ({data.adjuntos.length})
              </p>
              <div className="flex flex-wrap gap-2">
                {data.adjuntos.map((a) => a.es_imagen ? (
                  <a key={a.id} href={a.url} target="_blank" rel="noreferrer" title={a.original_name}>
                    <img src={a.url} alt={a.original_name}
                      className="h-24 w-24 object-cover rounded-lg border border-slate-200 dark:border-slate-700 hover:opacity-90 transition-opacity" />
                  </a>
                ) : (
                  <a key={a.id} href={a.url} target="_blank" rel="noreferrer" download={a.original_name}
                    className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-600 dark:text-slate-300 hover:border-[var(--brand-primary)]/40 transition-colors">
                    <FileText className="w-4 h-4 shrink-0 text-[var(--brand-primary)]" />
        ...
        <form onSubmit={enviarComentario} className="p-4 border-t border-slate-200 dark:border-slate-700 space-y-2">
          ...
          {comentFiles.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {comentFiles.map((f, i) => (
                <span key={i} className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 rounded-md px-2 py-1 text-[11px] text-slate-600 dark:text-slate-300">
                  {f.type.startsWith('image/') ? <ImageIcon className="w-3 h-3" /> : <FileText className="w-3 h-3" />}
                  <span className="max-w-[120px] truncate">{f.name}</span>
                  <button type="button" onClick={() => setComentFiles(comentFiles.filter((_, j) => j !== i))} className="text-slate-400 hover:text-red-500"><X className="w-3 h-3" /></button>
                </span>
              ))}
            </div>
          )}
          <div className="flex items-end gap-2">
            <label className="p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-[var(--brand-primary)] hover:border-[var(--brand-primary)]/40 cursor-pointer transition-colors" title="Adjuntar archivos">
              <Paperclip className="w-4 h-4" />
              <input type="file" multiple accept={ACCEPT_ADJUNTOS} className="hidden"
                onChange={(e) => { setComentFiles([...comentFiles, ...Array.from(e.target.files)]); e.target.value = '' }} />
            </label>
